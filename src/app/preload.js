...
const hakunekoAPI = {
    ...
    clipboard: {
        // Routed through IPC because the renderer's preload is sandboxed (sandbox defaults to
        // true since Electron 20), where require('electron') does not expose the clipboard module.
        readText: () => ipcRenderer.invoke('hakuneko:clipboard:readText'),
        writeText: (text) => ipcRenderer.invoke('hakuneko:clipboard:writeText', text),
    },
    browser: {
    ...
    ipc: {
        ...
        send: (channel, ...args) => {
            ...
            // Ephemeral response channels generated by ElectronBootstrap._ipcInvokeRenderer,
            // e.g. "hakuneko:ipc:connector-protocol:resp:1718234567890-0.123456789"
            const isDynamicResponseChannel = /^hakuneko:ipc:[a-z-]+:resp:[\w.+-]+$/.test(channel);
